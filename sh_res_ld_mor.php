<?php
    ...
    }else if($_POST['btnType'] == 'category'){
        $lodMorShCont = $_POST['lodMorShCont'];
        $currentDisSh = $_POST['currentDisSh'];
        $type_cat = $_POST['buttonType'];
        $masterBoxes = array();
        $sql = "SELECT * FROM main WHERE keywords LIKE '%{$type_cat}%' ORDER BY updated_date DESC LIMIT {$currentDisSh}, {$lodMorShCont}";
        $result_set_main = mysqli_query($conn, $sql);  // Rename to $result_set_main
        if($result_set_main){
            if(mysqli_num_rows($result_set_main) > 0){
                $noMorData = false;
    ...
    if($_POST['btnType'] == 'SearchResult'){
        $lodMorShCont = $_POST['lodMorShCont'];
        $currentDisSh = $_POST['currentDisSh'];
        $type_cat = $_POST['buttonType'];
        $masterBoxes = array();
        $sql = "SELECT * FROM Main WHERE template_name LIKE '%{$type_cat}%' ORDER BY updated_date DESC LIMIT {$currentDisSh}, {$lodMorShCont}";
        $result_set_main = mysqli_query($conn, $sql);  // Rename to $result_set_main
        if($result_set_main){
            if(mysqli_num_rows($result_set_main) > 0){
                $noMorData = false;
